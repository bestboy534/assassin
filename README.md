...
当前执行点：Plan 19 的任务 1-7 已全部交付并通过本地验证；Plan 20 Task 1-5 已完成，下一模块为 Task 6“前端定价与账单设置”。
...
| 20 | 套餐、计费与权益 | 部分实现（5/6） | 套餐权益、试用、Provider Webhook、套餐变更和用量计量已完成；七项指标、80%/100% 通知、硬限制拒绝和软限制超额告警已有测试；仍缺定价页、账单设置/用量/发票页面、统一权益提示和端到端账单流程 |
| 21 | 支持、状态页与管理后台 | 未开始 | 只有公开支持展示页；缺工单、诊断授权、状态事件和平台后台 |
| 22 | 可观测性与生产发布 | 未开始 | 缺结构化追踪、SLO、告警、性能门禁、灾备、发布流水线和生产验收 |

### 本次复核结果

- 后端：`92 passed`，Ruff 通过，Mypy 对 `150` 个源文件通过。
- 前端：TypeScript 类型检查通过，Vitest `88 passed`，Playwright Chromium 合规流程 `4 passed`，Vite 生产构建通过。
- 迁移：当前 head 为 `20260613_0020`，迁移集成测试通过；在 9 张计费表基础上为组织订阅增加待生效套餐、变更时间和变更类型字段。
...
- 计划清单：Plan 20 为 `checked=26`，其余计划仍为 `checked=0`；其中包含 21 个任务步骤和 5 条已满足的完成验收，勾选只代表对应项已有本地代码与验证证据。
- 未执行：仓库目前仍没有前端 lint、完整公开站/全业务 Playwright 矩阵、性能和 production-like staging 验收脚本；新增 CI 安全门禁仍需远程运行结果作为发布证据。

本次对 22 个计划逐项对照了目标、任务和完成验收，并交叉检查业务域、迁移、API 路由、前端路由、测试文件、CI 与 package scripts。结论是：Plan 19 的 7 个开发任务和 Plan 20 Task 1-5 已交付，但 22 个计划仍未完成全局生产验收；状态汇总为 `8` 个核心已验证、`12` 个部分实现、`2` 个未开始。
...
当前计划文档 checkbox 统计结果为 Plan 20 `checked=26`，其余计划 `checked=0`；完成度以本 README 的“计划完成状态”、代码交付和实际验证命令共同为准。
